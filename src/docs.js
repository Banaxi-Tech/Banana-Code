...
Banana Code is a terminal-native AI pair programmer that supports multiple providers (Gemini, Claude, OpenAI, DeepSeek, Kimi AI, Qwen, Ollama, LM Studio, llama.cpp, etc.) and gives the AI full autonomy via advanced tools.

## 💬 Slash Commands
Type these into the chat to control the app:
- /chats      : Open an interactive menu to resume previous chat sessions.
- /provider   : Switch between AI providers (gemini, claude, openai, deepseek, kimi, qwen, ollama, lmstudio, llamacpp, etc.).
...
- Add action@model or action@effort inside a prompt to open that menu before sending; the marker is removed from the prompt. action@effort is only supported for Claude and OpenAI Codex OAuth.
...
- Model Switch Tool: On supported providers, the AI can recommend temporarily using another model for the current turn. Banana Code asks before applying the switch. Future messages return to the configured model. OpenRouter, local Ollama, local LM Studio, and local llama.cpp are excluded.
- BananaSplit: Run cheaper local coding turns with Ollama, LM Studio, or llama.cpp, then call a non-local cloud model through 'bananasplit_review' to review and directly fix issues.
