...
import { DEFAULT_QWEN_BASE_URL, QWEN_MODELS } from '../constants.js';
...
export class QwenProvider {
    constructor(config) {
        this.config = config;
        this.baseURL = normalizeQwenBaseUrl(config.qwenBaseUrl);
        this.openai = new OpenAI({
            apiKey: config.apiKey,
            baseURL: this.baseURL
        });
        this.modelName = config.model || 'qwen3.6-plus';
    ...
    async autoRoute(message) {
        ...
        const models = QWEN_MODELS.map(m => ({
            id: m.value,
        ...
        return { model: 'qwen3.6-plus', reason: 'Auto-routing failed, using Qwen 3.6 Plus.' };
    }
    ...
    async sendMessage(input) {
        ...
        let spinner = null;
        if (!this.config.isApiMode) {
            spinner = ora({ text: getRandomSpinnerText('qwen'), color: 'yellow', stream: process.stdout }).start();
        }
        let finalResponse = '';
        ...
        try {
            while (true) {
                ...
                try {
                    ...
                    const errMsg = formatQwenError(e);
                    if (!this.config.isApiMode) {
                        console.error(chalk.red(`Qwen Request Error: ${errMsg}`));
                    }
                    return `Error: ${errMsg}`;
                }
            ...
            if (spinner && spinner.isSpinning) spinner.stop();
            if (!this.config.isApiMode) {
                console.error(chalk.red(`Qwen Runtime Error: ${err.message}`));
            }
